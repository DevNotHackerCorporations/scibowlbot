...
class Question(discord.ui.View):
    def __init__(self, ctx, subject: str):
        ...
        self.valid = ["PHY", "GEN", "ENERGY", "EAS", "CHEM", "BIO", "ASTRO", "MATH", "CS", "ES", "WEIRD", "CRAZY", "ALL"]
    ...
    async def run(self):
        ...
        if self.ctx.channel.id in self.ctx.bot.hasQuestion:
            raise BadArgument(f"There is already a question being served in this channel\n\nThink this is a mistake? Try running `.clear`")
        ...
        self.question_header = "**"+question_json["question"]["category"]+" "+question_json["question"]["tossup_format"]+" (Source: "+question_json["question"]["source"]+")\n**"
        ...
        if self.mc:
            ...
            if re.search(regstring, self.question.replace("\n", ""), flags=re.DOTALL | re.MULTILINE):
                for i in list(re.search(regstring, self.question.replace("\n", ""), flags=re.DOTALL | re.MULTILINE).groups()):
                    if i != "":
                        i = i.strip()
                        if len(i) < 70:
        ...
        self.alt_answers = self.generate_answers(self.correct_answer)
    ...
    async def on_timeout(self, element=False):
        if self.buzzed and not self.graded:
            ...
            for item in self.children:
                item.disabled = True
                if self.mc:
                    if item.label[0] == self.alt_answers[0][0]:
                        item.style = discord.ButtonStyle.green
                    elif item.label == "Buzz!":
                        item.style = discord.ButtonStyle.gray
                    elif item.label[0] != self.alt_answers[0][0]:
    ...
    def calc_timeout(self, string):
        5+(readtime.of_text(string).seconds)
    ...
    async def validate(self, answer):
        ...
        # Decide Verdict
        self.correct = True
        self.alt_answers = list(map(lambda x: x.upper(), self.alt_answers))
        accuracy = [self.compare(case, answer) for case in self.alt_answers]
